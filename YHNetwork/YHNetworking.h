 ...
 */
typedef void (^YHDownloadProgress)(int64_t bytesRead,
                                    int64_t totalBytesRead);

typedef YHDownloadProgress YHGetProgress;
typedef YHDownloadProgress YHPostProgress;
...
typedef NS_ENUM(NSUInteger, YHResponseType) {
    kYHResponseTypeJSON = 1, // 默认
    kYHResponseTypeXML  = 2, // XML
    // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
    kYHResponseTypeData = 3
};

typedef NS_ENUM(NSUInteger, YHRequestType) {
    kYHRequestTypeJSON = 1, // 默认
    kYHRequestTypePlainText  = 2 // 普通text/html
};
...
/*!
 *  @author 陈亦海, 15-11-15 13:11:27
 *
 *  请求成功的回调
 *
 *  @param response 服务端返回的数据类型，通常是字典
 */
typedef void(^YHResponseSuccess)(id response);

/*!
 *  @author 陈亦海, 15-11-15 13:11:59
 *
 *  网络响应失败时的回调
 *
 *  @param error 错误信息
 */
typedef void(^YHResponseFail)(NSError *error);
